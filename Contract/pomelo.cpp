...
void pomelo::assert_whitelist(string symbol, name contract)
{
...
void pomelo::assert_whitelist(symbol_type symbol, name contract)
{
	auto account = get_contract_name_by_symbol(symbol);
	eosio_assert(account == contract, "Transfer code does not match the contract in whitelist.");
}
...
name pomelo::get_contract_name_by_symbol(string symbol)
{
...
name pomelo::get_contract_name_by_symbol(symbol_type symbol) {
...
void pomelo::cancelbuy(name account, string symbol, uint64_t id) {
...
void pomelo::cancelsell(name account, string symbol, uint64_t id) {
	require_auth(account);
...
void pomelo::onTransfer(name from, name to, asset bid, std::string memo)
{
...
void pomelo::transfer(name from, name to, asset bid, std::string memo) {
	return;
}

void pomelo::setwhitelist(string symbol, name issuer) {
	require_auth(_self);
